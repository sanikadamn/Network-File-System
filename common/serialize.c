 ...
 * https://chat.openai.com/share/78af6e66-c17f-4591-8a61-66a98ef4e088. Code
 * below has been lightly edited.
 *
 *  AI-generated code for thread-safe, buffer-overflow-safe serialization, and
 * deserialization of strings in C.
 *
 *  Prompts:
 *  1. Write code for thread-safe buffer-overflow-safe serialization and
 * deserialization of strings in C.
 *  2. Make sure that it handles pathological cases.
 *  3. Make sure non-printable characters are appropriately escaped (and
 * unescaped) using hex representations.
 *  4. Do not write the main function.
 *  5. Allocate a new string inside the function instead of taking it as a
 * parameter.
 *  6. Escape backslashes correctly in the serialization and deserialization
 * procedures.
 */
...
str_t* serialize_string(const char* src) {
	...
	// Calculate the required buffer size for escaping non-printable
	...
	if (escaped_buffer == NULL || escaped_buffer->data == NULL) {
		...
		fprintf(
		    stderr,
		    "Error: Memory allocation failed during serialization\n");
	...
	for (size_t i = 0; i < input_length; ++i) {
		...
		if (src[i] < 32 || src[i] > 126 || src[i] == '\\') {
			...
			snprintf(&((char*)escaped_buffer->data)[j], 5,
			         "\\x%02X", src[i]);
		} else {
...
// Serialize a string into a new dynamically allocated buffer
str_t* serialize_buffer(buf_t buffer) {
	// Acquire the mutex lock for thread safety
	pthread_mutex_lock(&mutex);

	char* src = CAST(char, buffer.data);
	// Calculate the required buffer size for escaping non-printable
	// characters and backslashes
	size_t input_length = buffer.len;
	size_t escaped_length = 0;

	for (int i = 0; i < input_length; i++) {
		int escape_char =
		    (src[i] <= 32 || src[i] > 126 || src[i] == '\\');
		escaped_length += 4 * escape_char + 1 * !escape_char;
	}
...
// Unescape a string from a buffer with hex representations into a new
// dynamically allocated buffer
char* deserialize_string(const char* buffer) {
	// Acquire the mutex lock for thread safety
	pthread_mutex_lock(&mutex);

	// Calculate the required buffer size for unescaping hex representations
	// and backslashes
	size_t buffer_length = strlen(buffer);
	size_t unescaped_length = 0;

	for (size_t i = 0; i < buffer_length; ++i) {
		if (buffer[i] == '\\' && i + 3 < buffer_length &&
		    buffer[i + 1] == 'x') {
			// Found hex representation, needs unescaping
			unescaped_length += 1;
			i += 3; // Skip the next three characters representing
			        // the hex value
	...
	char* unescaped_buffer =
	    (char*)malloc(unescaped_length + 1); // +1 for null-termination

	if (unescaped_buffer == NULL) {
		// Handle memory allocation failure
		fprintf(
		    stderr,
		    "Error: Memory allocation failed during deserialization\n");
		pthread_mutex_unlock(&mutex);
		return NULL;
	}
